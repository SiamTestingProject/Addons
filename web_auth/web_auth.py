...
def project_image():
    return send_from_directory('photos', 'Zelretch.jpg')
...
def find_free_port() -> int:
    forced_port = os.environ.get("PORT") or os.environ.get("SPACE_PORT")
    if forced_port:
        try:
...
async def web_auth(api_id: int, api_hash: str, device_model: str) -> Tuple[bool, Optional[User]]:
    ...
    async def persist_authorized_session() -> None:
        from mongodb_storage import get_storage

        session_string = await client.export_session_string()
        get_storage(required=True).save_session_string(session_string)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if isinstance(signed_in, User):
                    await persist_authorized_session()
                ...
                logging.exception("RPC Error on sign_in: %s", e)
        ...
        while True:
            ...
            try:
                ...
                logging.exception("RPC Error on check_password: %s", e)
                error_message = 'Authorization error. Please try again.'
                current_step = 'password'
                continue

        signed_up = await client.sign_up(current_phone, sent_code_hash, "Zelretch")
        ...
        logging.exception("RPC Error during web authorization: %s", e)
        auth_result = None
        current_step = "error" 
        return False, None
    except Exception as e:
