...
def convert_module_new_format(file_path):
    ...
    content = re.sub(r"^from command import zel_command\s*\n?", "", content, flags=re.MULTILINE)
    content = re.sub(r"^import os\s*\n?", "", content, flags=re.MULTILINE)
    
    new_imports = "from command import zel_command\nimport os\n\n"
    ...
    content = re.sub(
        ...
        lambda m: f'zel_command({m.group(1)}{m.group(2)}{m.group(1)}, Module_Name, filename)',
        content
    )
...
def convert_module_filters_me(file_path):
    ...
    has_zel_sudo = "zel_sudo" in content
    has_who_message = "who_message" in content
    has_zel_command = "zel_command" in content
    ...
    if "from command import" in content and not (has_zel_sudo and has_who_message):
        content = re.sub(
            r'(from command import)([^\n]*)',
            lambda m: m.group(0) + 
                     ('' if has_zel_sudo else ', zel_sudo') + 
                     ('' if has_who_message else ', who_message'),
            content,
            count=1
        )
        # Обновляем флаги после добавления импортов
        has_zel_sudo = has_zel_sudo or "zel_sudo" in content
        ...
        def safe_replace(match):
            ...
            if not before.rstrip().endswith('~'):
                if after and after[0] in (' ', '&', '|', ')', '\n'):
                    return f'@Client.on_message({before}zel_sudo(){after}'
    ...
    if has_who_message:
        def add_who_message(match):
            ...
            # Проверяем, есть ли zel_command в декораторе
            needs_who_message = "zel_command" in decorator
