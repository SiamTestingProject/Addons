...
<code>{[your prefix]}theme help set text 🦊 Zelretch {version}
📦 Modules: {modules_count}
🔧 Prefix: {prefix}
❓ Use the inline button below</code>
</blockquote>
        """
    },
    "ru": {
...
• <code>{version}</code> - версия Zelretch
• <code>{modules_count}</code> - количество модулей
• <code>{prefix}</code> - префикс команды
• Кнопка списка команд добавляется автоматически
...
<code>{[ваш префикс]}theme info set text 🦊 Zelretch  {version}
Kurigram: {version}
🐍 Python {python_version}
⏰ Время работы: {uptime}
...
<code>{[ваш префикс]}theme help set text 🦊 Zelretch {version}
📦 Модули: {modules_count}
🔧 Префикс: {prefix}
❓ Используйте встроенную кнопку ниже</code>
</blockquote>
        """
    },
    "ua": {
...
• <code>{version}</code> - версія Zelretch
• <code>{modules_count}</code> - кількість модулів
• <code>{prefix}</code> - префікс команди
• Кнопка списку команд додається автоматично
...
<code>{[ваш префікс]}theme info set text 🦊 Zelretch  {version}
Kurigram: {version}
🐍 Python {python_version}
⏰ Час роботи: {uptime}
💻 Платформа: {platform}</code>
...
<code>{[ваш префікс]}theme help set text 🦊 Zelretch {version}
📦 Модулі: {modules_count}
🔧 Префікс: {prefix}
❓ Використовуйте вбудовану кнопку нижче</code>
</blockquote>
        """
    }
}
...
@Client.on_message(zel_command("theme", Module_Name, filename, "[help/info/vars] [set/reset] [image/text] [value]") & zel_sudo())
