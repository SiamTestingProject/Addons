...
class PluginValidator:
    ...
    def _convert_content(self, content, filename):
        ...
        if "from command import zel_command" not in content:
            ...
            for i, line in enumerate(lines):
                if line.strip() and not line.startswith(('import ', 'from ')):
                    insert_pos = i
                    break
            new_imports = ["from command import zel_command", "import os", ""]
        ...
        content = re.sub(
            ...
            lambda m: f'zel_command("{m.group(1)}", Module_Name, filename)',
            content
        )
